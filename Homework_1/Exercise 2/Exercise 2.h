...
void getField(const char* command, char* field, size_t& startIndex)
{
	size_t index = 0;
...
void sortString(Student* students, size_t studentsCount, const char* field)
{
	...
	if (strcmp(field, "name") == 0)
	{
		for (size_t i = 0; i < studentsCount; i++)
		{
			stringArray[i] = new char[strlen(students[i].getName()) + 1];
	...
	else if (strcmp(field, "email") == 0)
	{
		for (size_t i = 0; i < studentsCount; i++)
		{
			stringArray[i] = new char[strlen(students[i].getEmail()) + 1];
	...
	else if (strcmp(field, "gender") == 0)
	{
		for (size_t i = 0; i < studentsCount; i++)
		{
			stringArray[i] = new char[strlen(convertGenderToString(students[i].getGender())) + 1];
...
void sort(const char* command, Student* students, size_t studentsCount)
{
	...
	size_t startIndex = 0;
	getField(command, commandField, startIndex);
